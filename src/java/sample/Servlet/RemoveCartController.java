...
public class RemoveCartController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        /*getsession
        getcart from session
        trong cart chua iterms , get iterms trong cart,
        getpid 
        new dao
        getproductbyid
        cho 1 dong for duye theo iterms kiem tra id vs pid
        xoa iterm khoi iterms, break
        cart.setiterms
        session.setattibute(cart)
        seredierct(cartController)
         */
        ...
        for (Iterm iterm : list) {
            if (product.getProductID().equals(iterm.getProduct())) {
                list.remove(iterm);
                cart.setIterms(list);
                break;
            }
        ...
        for (Item item : list) {
            if(courseID.equals(item.getCourse().getCourseID())){
                list.remove(item);
                cart.setItems(list);
                break;
            }
